...
class TestJob(ProjectBase):
    ...
    def testVerifyJobListCount(self):
        ...
        email = 'nick.moore+auto2@mutualmobile.com'
        password = 'newpassword'

        self.caseId = 1302167
        self.navigation.navigateToLoginPage()
        self.authentication.login(email, password)
        oldJobCount = int(self.projectList.getProjectJobCount())
        self.projectList.selectAProject()
        self.jobList.tapCreateJob()
        self.job.enterJobName(self.job.generateRandomName())
        self.job.tapSubmit()
        ...
        self.assertion.assertEqual(newjobCount, oldJobCount + 1)

    @pytest.mark.ac
    def testVerifyJobNamesOnJobList(self):
        # Verify that job names are listed for each job on the job list
        email = 'nick.moore+auto3@mutualmobile.com'
        password = 'newpassword'
        ...
        self.job.enterJobName(self.job.generateRandomName())
        self.job.tapSubmit()
        jobCount = int(self.jobList.getJobCount())

        for i in range(jobCount):
            self.assertion.assertIsNotNone(self.jobList.getJobName(i))

    @pytest.mark.ac
    def testVerifyDateCreatedOnJobList(self):
        # Verify that Date Created is displayed for each job in the job list
        email = 'nick.moore+auto4@mutualmobile.com'
        password = 'newpassword'
    ...
    def testVerifyRecentJobListPosition(self):
        ...
        self.job.enterJobName(jobName)
        self.job.tapSubmit()
        jobNameInList = self.jobList.getJobName()

        self.assertion.assertEqual(jobName, jobNameInList)
